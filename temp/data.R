...
release$production_expansion_factor <- release$cwt_1st_mark_count / release$Total_Released
...
recovery <- recovery %>%
  mutate(size_limit = case_when(
                        location %in% c('FB', 'KO', 'KC') ~ 20,
                        location == 'SF' ~ 24,
                        location == 'MO' ~ 26),
         length_at_age = length)

write.csv("release", "./temp/sample_release.csv", row.names = FALSE)
write.csv("recovery", "./temp/sample_recovery.csv", row.names = TRUE)
